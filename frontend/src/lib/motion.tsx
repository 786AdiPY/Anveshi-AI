// Dependency-free scroll-motion primitives for the landing page. All respect
// prefers-reduced-motion: reveals show immediately, counters jump to target,
// and scroll-pinned sections fall back to a plain stacked layout.
import {
  createElement,
  useCallback,
  useEffect,
...
/** 0 → 1 progress of `ref` travelling through the viewport. `mode: 'pin'`
 * measures a tall section against its own scrollable length, for a
 * sticky/pinned child. */
...
/** Which of `ids` is the current chapter — scroll-position based rather than
 * IntersectionObserver, since the pinned pipeline section is many viewports
 * tall and would otherwise stay "active" across its whole length. */
...
/** Editorial line-by-line reveal: each word lifts in sequence. */
  ...
  return createElement(
    ...
    words.map((w, i) =>
      createElement(
        ...
        createElement("span", { className: "rw__i", style: { ["--rw-i" as string]: start + i } }, w),
      ),
    ),
  );
}
...
export function CountUp({ to, from = 0, decimals = 0, duration = 1400, prefix = "", suffix = "", className }: CountUpProps) {
...
/** Reveals lines one at a time once in view — used for the terminal playback. */
...
/** Cursor-driven 3D tilt with spring-damped easing. Writes CSS custom
 * properties (`--rx`/`--ry` rotation, `--gx`/`--gy` specular highlight,
 * `--sx`/`--sy` shadow offset, `--lift` proximity elevation) so CSS does the
 * painting. No-ops for reduced-motion and touch-only pointers. */
...
/** Subtle cursor-follow translate for a primary CTA. */
